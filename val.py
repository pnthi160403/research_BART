...
def validate(model, config, beam_size, val_dataloader, num_example=20):
    ...
    with torch.no_grad():
        ...
        for batch in batch_iterator:
            ...
            # recall, precision
            recall_metric.add(predictions=pred_ids.detach().cpu().numpy().astype(int), references=label_ids.detach().cpu().numpy().astype(int))
            precision_metric.add(predictions=pred_ids.detach().cpu().numpy().astype(int), references=label_ids.detach().cpu().numpy().astype(int))
